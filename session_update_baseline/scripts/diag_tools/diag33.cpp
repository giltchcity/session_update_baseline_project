...
int main(int argc, char** argv) {
  if (argc < 3) {
    std::cerr << "usage: diag33 <map.4dmap> <stamp_idx> [stamp_idx2 ...]\n";
    return 2;
  }
  ...
  for (int a = 2; a < argc; ++a) {
    ...
    for (const auto& [id, node] : dsg->getLayer(spark_dsg::DsgLayers::OBJECTS).nodes()) {
      ...
      std::cout << "  node=" << id << " sem=" << attrs->semantic_label
                ...
                << " first=" << (attrs->first_observed_ns.empty() ? -1 : (long)(attrs->first_observed_ns.front() / 1000000000ULL))
                << "s last=" << (attrs->last_observed_ns.empty() ? -1 : (long)(attrs->last_observed_ns.front() / 1000000000ULL)) << "s\n";
    }
  }
  return 0;
}

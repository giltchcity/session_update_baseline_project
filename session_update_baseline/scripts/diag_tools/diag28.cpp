...
int main(int argc, char** argv) {
  if (argc < 2) { std::cerr << "usage: diag28 <map.4dmap> [inst_id]\n"; return 2; }
  auto map = khronos::SpatioTemporalMap::load(argv[1]);
  if (!map) { std::cerr << "load failed\n"; return 2; }
  long target = (argc > 2) ? std::atol(argv[2]) : 2;
  const auto stamps = map->stamps();
  size_t idx = 0;
  for (const auto& st : stamps) {
    const auto dsg = map->getDsgPtr(st);
    if (!dsg || !dsg->hasLayer(spark_dsg::DsgLayers::OBJECTS)) { ++idx; continue; }
    for (const auto& [id, node] : dsg->getLayer(spark_dsg::DsgLayers::OBJECTS).nodes()) {
      ...
      if (!nv) { std::cout << idx << " t=" << (st/1000000000ULL) << "s inst=" << inst << " verts=0\n"; continue; }
      ...
      std::cout << idx << " t=" << (st/1000000000ULL) << "s inst=" << inst << " verts=" << nv
                << " min=(" << mn.x() << "," << mn.y() << "," << mn.z() << ")"
                << " max=(" << mx.x() << "," << mx.y() << "," << mx.z() << ")"

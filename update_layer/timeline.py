...
def _np(x):
    """Host numpy copy of a torch tensor (GPU layer) or numpy array (reference layer)."""
    if x is None:
        return None
    if hasattr(x, "detach"):
...
class LayerTimeline:
    def __init__(self, res):
        ...
        try:
            import torch
            ids_t = torch.arange(n, device=el.cnt.device) if hasattr(el.cnt, "device") else ids
        except ImportError:
            ids_t = ids
        self.el_xyz = (_np(el.sum[:n]) / _np(el.cnt[:n])[:, None]).astype(np.float32)
        self.el_label = _np(el.label[:n]).astype(np.uint32)
        self.el_normal = _np(el.normals(ids_t)).astype(np.float32)
        self.el_birth = _np(el.birth[:n]).copy()
        self.el_death = _np(el.death[:n]).copy()
        self.el_inherited = _np(el.inherited[:n]).copy()
        self.el_identity = _np(el.identity[:n]).copy()
        self._stamps: List[int] = list(res.stamps)
        self.display = res.display
        self.frag_points = {uid: _np(f.points) for uid, f in res.fragments.items()}
        self.frag_birth = {uid: f.birth_time for uid, f in res.fragments.items()}
        self.frag_normals = {uid: (_np(f.normals) if f.normals is not None

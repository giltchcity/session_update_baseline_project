...
## 2026-09-16：Synthetic AB 机制修复与验收

### 最新完整验证：观测相容性、粗背景关联、空间覆盖和真实观测时间

以下较早记录保留为审计历史；当前完整地图为 `runs/v37_surface_final_evidence_20260916`。

```text
Synthetic AB 本轮修复结果：运动帧分段与旧背景关联
已完成完整 A→B；输入仍为唯一原始 30 Hz、680×480，A4186+B3555=7741帧。
背景10cm、物体5cm、粗时序检查；没有重做数据，没有用GT物体运动/网格指导选帧或删除；GT相机位姿、语义/实例仍为原设定输入。

树选帧根因：最终树 track 没有动态标志或运动历史，未触发 static_fallback；该段漏在运动检测/关联，旧选帧只依赖运动标志因而接收全部帧。
修复：同一 physical instance 的静态融合增加双向实际深度相容性检查，独立于D1分类门槛；视野外、缺深度、近遮挡不作为分段证据。
旧背景修复：候选关联范围采用粗背景体素对角线；后续深度的空位判据仍为半体素，不随关联范围放宽。
整体关闭要求独立表面的实际空位覆盖，默认20%，对所有对象相同；重复观测不增加空间覆盖。D2新位置接管路径保留。
最后支持时间采用真实 tracker.last_seen，不会因重建缓存少存一帧而倒退；检查时刻也不能冒充观测时刻。
存在区间的真实像素覆盖修复也包含在本轮结果中。

Session A：580.8 MiB
I7 bed_0000 已见表面5cm距离召回：98.80% → 98.80%
I49 cabinet_0001 已见表面5cm距离召回：99.40% → 99.40%
I68 chair_0000 已见表面5cm距离召回：99.05% → 99.05%
I69 chair_0001 已见表面5cm距离召回：95.47% → 95.47%
I109 flower_0003 已见表面5cm距离召回：94.33% → 94.33%
I121 ornament_0004 已见表面5cm距离召回：46.15% → 46.15%
I165 table_lamp_0000 已见表面5cm距离召回：99.69% → 99.69%
I108 flower_0002 已见表面5cm距离召回：52.10% → 68.51%
旧位置 I69/0：物体残留 0→0；背景残留 0→0；有后续空位观测的背景残留 0→0
旧位置 I69/1：物体残留 0→0；背景残留 1→0；有后续空位观测的背景残留 1→0
旧位置 I69/2：物体残留 0→0；背景残留 66→62；有后续空位观测的背景残留 23→19

表面精度下降超过2个百分点的对象（未全量验收）：[{"id": 164, "name": "table_0003", "before": 0.9046849757673667, "after": 0.8807212205270458, "delta": -0.023963755240320905}]
Session B：1127.0 MiB
I7 bed_0000 已见表面5cm距离召回：99.09% → 99.09%
I49 cabinet_0001 已见表面5cm距离召回：92.12% → 92.12%
I69 chair_0001 已见表面5cm距离召回：98.19% → 98.19%
I121 ornament_0004 已见表面5cm距离召回：46.38% → 46.38%
I165 table_lamp_0000 已见表面5cm距离召回：100.00% → 100.00%
I108 flower_0002 已见表面5cm距离召回：64.42% → 75.86%
I109 flower_0003 已见表面5cm距离召回：68.27% → 87.53%
旧位置 I7/0：物体残留 0→0；背景残留 116→111；有后续空位观测的背景残留 15→10
旧位置 I49/0：物体残留 0→0；背景残留 1→0；有后续空位观测的背景残留 1→0
旧位置 I68/0：物体残留 0→0；背景残留 4→4；有后续空位观测的背景残留 4→4
旧位置 I68/1：物体残留 0→0；背景残留 83→80；有后续空位观测的背景残留 47→46
旧位置 I69/0：物体残留 0→0；背景残留 7→7；有后续空位观测的背景残留 7→7
旧位置 I69/1：物体残留 0→0；背景残留 0→0；有后续空位观测的背景残留 0→0
旧位置 I69/2：物体残留 0→0；背景残留 6→0；有后续空位观测的背景残留 6→0
旧位置 I69/3：物体残留 0→0；背景残留 2→2；有后续空位观测的背景残留 2→2
旧位置 I109/0：物体残留 0→0；背景残留 22→0；有后续空位观测的背景残留 22→0
旧位置 I121/0：物体残留 0→0；背景残留 0→0；有后续空位观测的背景残留 0→0
旧位置 I165/0：物体残留 0→0；背景残留 0→0；有后续空位观测的背景残留 0→0

表面精度下降超过2个百分点的对象（未全量验收）：[{"id": 164, "name": "table_0003", "before": 0.9412698412698413, "after": 0.9077757685352622, "delta": -0.03349407273457905}]
完整B地图中新树正面深度吻合像素：22.62% → 46.60%
24项源码回归通过。完整地图已评估，但不能宣称所有叶片完整或全场景零残留。
距离召回率不等于逐像素表面完整度。残留以2.5cm评估采样去重，旧GT≤5cm且离当前全部GT>10cm，电视贴墙案例排除。
未实现物体刚体运动估计与旧网格搬运；本轮修的是静态融合的输入相容性。
完整指标：/home/jixian/Desktop/FT/datasets/synthetic_ab/reports/v37_range15_diagnosis/surface_final_visibility_metrics_a.json 和 _b.json
地图目录：/home/jixian/Desktop/FT/datasets/synthetic_ab/runs/v37_surface_final_evidence_20260916
图：Synthetic_ABC/TREE_REPAIR_COMPARISON.png
```



源码仍是本仓库 `session_update_baseline/`，运行入口为
`build_canonical/session_khronos_node`。当前是 **V37 加修复的工作树**，不是未修改的
历史 V37。真实 ABC 的已接受结果见下一节；本节结果不可冒充真实 ABC 的结论。

### 唯一输入与当前输出

- 数据根目录：`/home/jixian/Desktop/FT/datasets/synthetic_ab`。
- 原始输入保持 30 Hz、680×480：A 4,186 帧，B 3,555 帧；image-scale=1。
  不复制 10 Hz 数据，不重渲染、不改 USD 场景。GT pose、GT semantic、GT instance 输入。
- 当前完整修复验证：`runs/v37_surface_final_evidence_20260916`，**全帧 A→B 已完成；树旧残留清除，仍有其他质量限制，未宣称全场景零残留**。
- 粗机制配置：背景 10 cm、物体 5 cm；每 25 个 backend update 检查（约 10 s），
  结束时强制处理。原生默认检查间隔仍为 5。粗时序不改变 30 Hz 输入。
- 每次 B 继承本次 A。原始有缺陷参考保留在 `runs/v37_range15_20260916`。
- 全部证据与指标：`reports/v37_range15_diagnosis/`；运行状态入口：
  `reports/ACTIVE_RUN.json`。源码测试通过不等于地图已通过 GT 验收。

### 已复现并处理的问题

1. **真实空位证据没有进入原网格射线索引。** I49 用相同 183 个 RGB-D 源帧，原索引
   没有决定性证据，实际像素投影有缺席证据。B 第 1872 帧旧点距离 2.574172 m，
   实测背景 2.876 m，背景顶点距实际命中点仅 3.18 mm。其 first_seen=201.733333333 s，
   last_seen=203.866666667 s；减去 3 s active window 后末端早于起点，导致没有射线。
   不是“旧位置后面没有建出背景”。现在在索引无决定性证据或拟删除时复核实际像素。
2. **错误的时间和覆盖条件。** 物体出现以前的空位不得删除后来状态；无对应帧、
   视野外、近处遮挡不得投缺席票。查询必须在该状态最后支持之后且不超过当前时刻。
3. **30 cm 同物体匹配容差误用为遮挡容差。** A 的 I49：实测 5.279 m、查询 5.578538 m，
   原来竟被算背景替代。异物更近就是遮挡，仅保留范围存储毫米量化的 1 mm 余量。
   同一案例 3 条假反证变 0；完整 A 的柜子已见表面覆盖率恢复至 99.40%（5 cm）。
4. **恢复原 V37 D2 接管分支。** 我此前扩大“无新候选也能凭缺席关闭”的条件时，
   错误漏掉原有“新候选已观测、旧位置无支持”的接管路径。该回归已纠正。
   无候选又无缺席证据仍保留，几何网格重叠不作为独立的实测支持票。
5. **终局先判断后登记，最后新位置没有参与判断。** 保存前 canonicalize 会接收最后
   一批片段，先前随后直接保存。现在登记后再做一次终局状态处理，只保存一份终局快照。
   `terminal` 完整链的 A I69：旧位置物体层残留 367→0 个 2.5 cm 采样格；
   新位置已见表面覆盖率 0→95.47%（5 cm），99.48%（10 cm）。
6. **对象关闭但背景副本未清除。** 背景顶点标签全部未知，不能按物理 ID 直接删除。
   使用关闭状态的表面对应和真实后续深度，不使用 GT 或包围盒整体删除。
   原来只接受早于继承 A 对象时间戳的背景面，漏掉 B 阶段再次观测的旧位置。
   当前按每个背景面自身时间查询，并保护状态关闭后新建的表面。
   背景几何与物体身份判据分开：任意身份的同位置实测表面都支持几何存在；
   只在最后几何支持之后看到超过半个背景体素的实际空位，才允许清除。
   此项全链 GT 结果见 `CURRENT_REPAIR_RESULT.json`，不能宣称残留已经全部清干净。

### GT 验收口径与已纠正的误判

- I68 在 B 第 1798 帧（59.933333 s）USD render visibility 变为 invisible 并保持隐藏。
  最终无该椅子是正确结果。早期只看变换而忽略 visibility，把它错算成误删。
  使用 `*_visibility_metrics_*.json`，不要引用旧 `*_surface_metrics_b.json` 的隐藏物体召回率。
- 完整度分母是最终可见 GT 位姿下 RGB-D 实际看到的表面，不是看不到的背面。
  指标粗采样为每 15 帧、像素步长 4、表面去重 2.5 cm；它不改变建图输入。
- 旧残留必须距旧 GT ≤5 cm 且距所有当前可见 GT >10 cm；再报告实际后来深度穿透。
  “离旧物体中心近”不构成残留。电视贴墙案例未计入此旧位置验收。
- 粗物体网格是 5 cm，植物叶片/细小装饰仍有几何损失；不能承诺所有指标优于细配置。
  I109 最后新位置只有 89 帧可见（约 3 s）；这也不能直接证明所有缺失都是观察不足。
- 核心 15/15、运行层 8/8 已通过，包含真实 ABC 同会话/跨会话等价与几何来源回归。
  新增遮挡、时间边界、无像素覆盖、终局接管、背景重新出现保护测试。

### 资源与清理

已删除明确淘汰的 5 个 Synthetic 生成 final.4dmap，释放 53.76 GiB；
记录为 `reports/v37_range15_diagnosis/removed_superseded_maps.json`。
原始输入、真实 ABC、源码和运行日志保留。释放的是 WSL 文件系统空间，不代表 VHDX 自动缩小。
细配置首轮 A/B 约 10/15 GiB；目前粗配置 A/B 约 0.56/1.1 GiB，最终大小以新结果为准。

### 此前 geometry 轮的完整链结果（历史记录）

- A：574.1 MiB。I69 已见表面覆盖率 5 cm=95.47%；I109 10 cm=99.39%。
  I69 各旧状态 (物体残留格,背景残留格,有后续空位观测残留格)：[(0, 0, 0), (0, 1, 1), (0, 66, 23)]。
- B：1114.4 MiB。I69 已见表面覆盖率 5 cm=98.19%；I109 10 cm=94.20%。
  I69 各旧状态 (物体残留格,背景残留格,有后续空位观测残留格)：[(0, 7, 7), (0, 0, 0), (0, 6, 6), (0, 2, 2)]。
  I109 各旧状态 (物体残留格,背景残留格,有后续空位观测残留格)：[(0, 22, 22)]。

表面格为 2.5 cm 评估采样，不是建图体素。多个旧状态可重叠，不能把它们简单相加当作场景唯一残留数。
详细指标在 `reports/v37_range15_diagnosis/CURRENT_REPAIR_RESULT.json` 和同名 TXT。
播放器默认入口现在读取 `reports/CURRENT_VIEW.json`，与正在运行的 ACTIVE_RUN 分开。
已打开的 terminal 对照窗口不会被自动关闭或替换。
另删除 10 个淘汰的粗地图，释放 7.96 GiB；保留原始细参考、terminal 对照、当前结果及全部日志/指标。
记录：`removed_failed_coarse_maps.json`。

### I109 新位置的采集覆盖核查

对 B 最终位姿可见的全部 89 帧（3466–3554，约 2.97 s）逐帧读取原始深度、实例图与位姿：
相机完整 4×4 位姿最大差仅 1.11e-16，没有产生新的视角；其中 88 帧树深度和树实例掩码
与最后一帧逐元素完全相同。树没有被画面裁掉。树实例区域合计 1,150,401 个深度像素，
无无效/超范围值，全部距对应 GT 三角面小于 2 cm；95 分位误差约 1e-6 m。
因此已确认的输入限制是移动后固定视角，尚无证据表明这部分深度/位姿生成错误。
这能解释没有侧面/背面观测，不能解释已见表面分母下的所有重建损失。
细配置与粗配置历史运行不止分辨率不同，不能把 92.6% 与 68.3% 的全部差异因果归给分辨率。
采集核查：`reports/v37_range15_diagnosis/tree_capture_coverage.json`。
后续若补数据，应改变相机观察方向，而不是仅延长原地重复帧；本次未修改原始数据或 USD。

### 相机、树正面和证据时间审计（2026-09-16 补充）

本次是原始 RGB-D 的局部受控回放与代码修复，不是另一轮完整 A/B。
最新源码已构建安装，核心 15/15、baseline 8/8 测试通过；
`runs/v37_geometry_evidence_20260916` 仍是修复这两处新增问题之前的完整地图。
不要把下面的局部回放或测试当成全链 GT 验收。

1. 相机 680×480，fx=fy=588.897289890142，cx=340，cy=240；水平 FOV 60°，
   垂直 44.3459°。pose 为 world_T_camera，robot_T_sensor 为单位变换；
   原始深度是米单位光轴 Z，Hydra 转径向距离后才用于证据比较。
   树最后一帧按渲染像素中心反投影与 GT 的 95 分位误差约 1e-6 m。
   Hydra 整数像素中心与渲染器相差半像素，95 分位误差约 4.57 mm；
   单独调整该项的回放未改善树缺失。未修改生产 K 或原始数据。
2. 最后约 3 秒确实参与重建，当前树物体网格为 6,720 顶点；不是没有生成。
   用实际 canonical MeshObjectExtractor、InstanceForwarding 和同一 RGB-D 控制变量回放：
   混合运动/静止帧，5 cm 体素，已见表面 5 cm 距离召回率 68.27%；
   只用最终静止帧，同样 5 cm 体素为 82.84%；
   混合帧改 2 cm 为 92.59%，静止帧改 2 cm 为 97.41%。
   静止段 30 帧与全部 88 帧的网格和指标一致。GT 只用于评估，不参与重建。
   混合回放有 72 个有效帧而原记录 73 个，但粗、细网格顶点数与召回率均复现原结果。
3. 上述距离召回率不是图像逐像素填满程度。沿最终真实树像素发射射线，
   命中网格且深度误差不超过 5 cm 的比例分别为：混合/5cm 22.62%，
   静止/5cm 43.67%，混合/2cm 28.80%，静止/2cm 56.60%。
   即使局部细化，仍不能宣称叶片完整。图见 Windows Synthetic_ABC/TREE_FRONT_AUDIT.png。
4. 已修漏掉的物体存在区间入口：checkObjectObservation 原来仍调用旧 checkPhysical，
   可以把没有像素覆盖的网格代理射线记成缺席。现在使用 checkPhysicalObserved，
   要求实际覆盖并检查深度/遮挡，同时保留原 V37 对不同身份的保守存在性投票。
   新回归包含视野外、缺帧和真实空位，修复前确实失败，修复后通过。
5. 已修支持时钟：过去将检查时刻当成最后支持时刻，可能跳过真正后续空位。
   现在传递实际支持帧 latest_support_stamp，分开保存几何观测时间与已确认支持时间；
   20 秒检查到的 5 秒支持不会再屏蔽 15 秒空位。继承 A 的几何时间不被 B 检查时间覆盖。

尚未解决/尚待验证：

- 新树重建受混合运动帧影响已由回放确认。源码中静态提取只在运动标志/时间有效时
  筛掉运动前帧，低于位移门槛的静态回退会清除这些标志。但现有日志不足以区分
  本例是 tracker 没标出运动还是回退清掉标志，不能宣称精确分支已证明或已修好。
  后续应记录提取入口的运动标志、最后运动时刻和实际选帧，按实测稳定状态分段；
  不允许硬编码树 ID、GT 停止时刻或最后 3 秒。
- 旧树物体层残留为 0，但背景层仍有 22 个旧表面采样格，且全部有后来真实空位。
  对应 25 个原始顶点与旧物体私有网格距离 5.08–10.37 cm，现有 5 cm 匹配漏掉它们。
  不能直接无限扩大删除半径；需要在粗背景尺度下重新关联并保留真实深度保护。
- 固定视角限制背面覆盖，薄叶片的 TSDF/网格损失仍存在；不能把全部正面缺失归给数据。
- 本次两处证据修复尚未重新执行完整 A/B 和 GT 验收。原始 30 Hz 数据保持不变。

证据文件：`tree_reconstruction_controlled_audit.json`、`camera_geometry_audit.json`、
`tree_capture_coverage.json`、`geometry_tree_residual_association.json`、
`reducer_coverage_before.log`、`build_audit_guards.log`，均在 `reports/v37_range15_diagnosis/`。
...
- `persistent` 和 `new` 进入当前地图；`absent` 只有在可靠重观测后移除；`unobserved` 保守
  保留。没有看到不等于确认消失。
- 当前代码仅从 background TSDF 排除 dynamic semantics 和 motion pixels；静止家具也融合进
  背景，同时有独立的 object private mesh。旧文档“所有 object 像素均排除”不符合当前 V37
  的 `ActiveWindow::updateMap`。物体层与背景层必须按观测一致更新，不能依赖 viewer 隐藏旧表面。
  2026-09-16 的联动修复见上文。跨 segment 的当前物体几何由 `PersistentObjectState` 维护。
...
### 合成数据生成地图保留规则（2026-09-16）

按用户要求，后续合成数据跑图及时清理已淘汰的大型生成地图。新一轮完整运行、评估并更新 `datasets/synthetic_ab/reports/CURRENT_VIEW.json` 后，保留当前可查看的 A/B `final.4dmap`；删除已被替代、且未被运行中进程使用的旧合成 `.4dmap`。先检查路径及当前地图是否完整，逐文件删除并记录文件大小和路径。保留原始 30 Hz 数据、USD/GT、源码、日志、指标与诊断导出，不触及真实 ABC 地图。当前可查看结果不等于全部质量问题已验收。清理地图文件释放的是磁盘空间；不自动终止进程、关闭 WSL 或压缩虚拟磁盘。
